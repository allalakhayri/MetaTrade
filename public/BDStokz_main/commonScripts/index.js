...
async function fetchDataAndPopulateTable() {
  ...
  getElementAndReplaceWithCloneAbrar("mainProfileUser").href = "../position/position.html?name="+userDataset.username;
    try {
        const response = await fetch('http://localhost:3000/stock/order/gain'); 
    ...
    try {
      const response2 = await fetch('http://localhost:3000/stock/order/lose'); 
      const data2 = await response2.json();
      ...
      if (data2) {
        ...
        const response22 = await fetch('http://localhost:3000/stock/'+data2[0].SYMBOL); 
